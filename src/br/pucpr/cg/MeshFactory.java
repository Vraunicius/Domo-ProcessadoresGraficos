...
public class MeshFactory {
    ...
    public static Mesh loadTerrain(File file, float scale, int texRepeat) throws IOException {
        ...
        return new MeshBuilder()
                    ...
                    .addVector4fAttribute("aTexWeights", texWeights)
    ...
    static Mesh createDome(int width, int depth, int texRepeat){
        float hw = width / 2.0f;
        float hd = depth / 2.0f;
        ...
        for (int z = 0; z < depth; z++) {
            for (int x = 0; x < width; x++) {
                float h = (float)(Math.sin((new Vector2f(x,z).length())));
                positions.add(new Vector3f(x - hw, -h, z - hd));
            }
        }
    ...
    /*static Mesh createDome2(int slices, int stacks) {
        List<Vector3f> vertices = new ArrayList<>();
        List<Vector2f> texCoords = new ArrayList<>();
